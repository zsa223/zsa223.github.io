...
2. **`flatMapping()`**:
   ...
   - **`Collectors`**와 함께 사용되며, 주로 **그룹화**나 **집계** 연산에서 **스트림을 평탄화**하여 데이터를 수집할 때 사용됩니다.
   - 각 그룹 내에서 중첩된 데이터를 평탄화한 후, 이를 다시 그룹화하거나 집계하는 데 적합합니다.
...
- **`flatMap()`**은 **스트림 내에서** 중첩된 리스트나 배열을 평탄화하여 개별 요소로 변환할 때 사용됩니다. 즉, 계층화된 데이터를 하나의 스트림으로 변환하고, 그 데이터를 처리하는 데 유용합니다.
- **`flatMapping()`**은 **`Collectors`**와 함께 사용되며, **데이터를 그룹화한 후** 각 그룹 내에서 **데이터를 평탄화**한 다음 집계 작업을 수행할 때 사용됩니다. 이는 특히 **그룹 내에서 데이터 변환 및 수집 작업**이 필요할 때 매우 유용합니다.
